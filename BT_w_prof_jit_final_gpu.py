...
#    model = ipex.optimize(model)
    ...
    encoding = processor(images[0], texts[0], return_tensors="pt").to(device)
    model = model.to(device)
    if jit:
        model = torch.jit.trace(model,example_kwarg_inputs=dict(encoding), check_trace=False, strict=False)
        model = torch.jit.freeze(model)        
    ...
    model = model.to(device)
    encoding = processor(image, texts[0], return_tensors="pt").to(device)
#    model = ipex.optimize(model)
    if jit:
        model = torch.jit.trace(model,example_kwarg_inputs=dict(encoding), check_trace=False, strict=False)
        model = torch.jit.freeze(model)
...
if profile:
    ...
    def trace_handler(p):
        ...
        if jit:
            with open("./profile/"+ task + '_gpu_' + timestr + '_jit.csv', 'w+',newline='') as file:
                file.writelines(output)
            output = p.key_averages(group_by_input_shape=True).table(sort_by="cuda_time_total")
            with open("./profile/"+ task + '_gpu_' + timestr + '_byshape_jit.csv', 'w+',newline='') as file:
                file.writelines(output)
        else:
            with open("./profile/"+ task + '_gpu_' + timestr + '.csv', 'w+',newline='') as file:
                file.writelines(output)
            output = p.key_averages(group_by_input_shape=True).table(sort_by="cuda_time_total")
            with open("./profile/"+ task + '_gpu_' + timestr + '_byshape.csv', 'w+',newline='') as file:
                file.writelines(output)
